"""This script helps to convert yolo-style data to the coco format.

Usage:
    $ python yolo2coco.py /path/to/data # image_dir

Note:
    1. Before running this script, please make sure the root directory
    of your data is formatted in the following struction:
    .
    └── $ROOT_PATH
        ├── classes.txt
        ├── labels
    ...
    data. The image paths in these files must be ABSOLUTE paths.
    3. Once the script finishes, the result files will be saved in the
    directory named 'annotations' in the root directory of your data.
...
def organize_by_existing_files(image_dir: str, existed_categories: list):
    ...
    for cat in categories:
        if cat in existed_categories:
            ...
            print(f'Start to read {cat} data definition')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'image_dir',
        type=str,
        help='data directory with ./images and ./labels, classes.txt, etc.')
