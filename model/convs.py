...
class SC(nn.Module):
    def __init__(self, c1, c2, k=3, s=2, p=1, g=1, d=1, act=True, length=4, *args, **kwargs):
        super(SC, self).__init__(*args, **kwargs)
        self.length = length
        c_ = c2 // 2  # hidden channels
        self.cv1 = Conv(c1, c_, k, s, 1, g, act=act)
...
if __name__ == "__main__":
    x = torch.randn(1, 64, 128, 128)   # 输入: batch=1, 通道=64, 尺寸=128x128
    model = SC(c1=64, c2=128, length=4)
...
if __name__ == '__main__':
    ...
    m = SELayer(channels=128)
    y = m(x)
    print(y.shape, "SELayer")
    pass
...
if __name__ == '__main__':
    x = torch.rand(4, 128, 64, 64)
    m = ECALayer(channels=128, k_size=3)
    y = m(x)
    print(y.shape, "ECALayer")
    pass
...
class CBAM(nn.Module):
    def __init__(self, channels: int, kernel_size: int = 7):
        ...
        self.ca_fc = nn.Conv2d(channels, channels, kernel_size=1, stride=1, padding=0, bias=True)
        self.ca_act = nn.Sigmoid()

        assert kernel_size in (3, 7), "kernel_size must be 3 or 7"
        padding = 3 if kernel_size == 7 else 1
        self.sa_conv = nn.Conv2d(2, 1, kernel_size=kernel_size, padding=padding, bias=False)
        self.sa_act = nn.Sigmoid()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ca = self.ca_act(self.ca_fc(self.ca_pool(x)))
        x = x * ca

        avg = torch.mean(x, dim=1, keepdim=True)
        mx = torch.max(x, dim=1, keepdim=True)[0]
        sa = self.sa_act(self.sa_conv(torch.cat([avg, mx], dim=1)))
        x = x * sa
...
class ODConv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=3, stride=2, padding=1, dilation=1, groups=1,
...
if __name__ == '__main__':
    x = torch.rand(4, 128, 64, 64)
    m = ODConv2d(in_planes=128, out_planes=128)
    y = m(x)
    print(y.shape, "ODConv2d")
    pass
...
if __name__ == '__main__':
    x = torch.rand(4, 128, 64, 64)
    m = FDConv(in_channels=128, out_channels=64, kernel_size=3, padding=1, bias=True,stride=2)
    y = m(x)
    print(y.shape)
    pass
